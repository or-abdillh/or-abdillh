<template>
	<section id="about" class="py-8 bg-gradient-to-b from-gray-50 to-indigo-50 px-8 md:px-20">
		<h1 data-aos="fade-up" class="mb-8 text-3xl text-center text-indigo-700 font-bold">About Me</h1>
		...
		<section class="mt-6 w-full justify-between flex flex-wrap lg:justify-center  gap-6 md:mt-10 gap-6">
			<div class="md:w-5/12 ">
				<h2 data-aos="zoom-in" class="mb-5 font-medium md:text-center text-indigo-700 text-xl">Educations</h2>
				<div>
					<template v-for="(edu, x) in educations" :key="x">
						<div data-aos="zoom-in" class="mb-3 border-l-4 border-indigo-600 rounded py-2 pl-3">
							<span class="flex items-center gap-3">
								<i class="fas fa-building text-indigo-600"></i>
								<p class="font-medium text-gray-700">{{ edu.name }}</p>
							</span>
			...
			<div class="md:w-5/12">
				<h2 data-aos="zoom-in" class="mb-5 md:text-center font-medium text-indigo-700 text-xl">Skills</h2>
				<ul>
					<template v-for="(skil, x) in skils" :key="x">
						<li data-aos="zoom-in" class="mb-3 border-l-4 border-indigo-600 rounded py-2 pl-3 font-medium text-gray-700">{{ skil }}</li>
					</template>
				</ul>
			</div>
			
			<div class="lg:w-10/12">
				<h2 data-aos="zoom-in" class="mb-5 md:text-center lg:mb-10 font-medium text-indigo-700 text-xl">Languages & Tools</h2>
